    ...
    <div className="card p-3">
      ...
      >
            ...
            <div key={ingredientId} className="col-sm-10 offset-2">
              <div className={"row mb-3 " + (i === ingredients.length - 1 ? "" : "border-bottom")}>
                <div className="col-md-6">
                  <input type="text" name={"ingredient-name-" + i} className="form-control d-inline-block mr-2 mb-2" placeholder="Név" defaultValue={ingredient.name} required />
                </div>
                <div className="col-md-2">
                  <input type="text" name={"ingredient-quantity-" + i} className="form-control d-inline-block mr-2 mb-2" placeholder="Mennyiség" defaultValue={ingredient.quantity} required />
                </div>
                <div className="col-md-2">
                  <select name={"ingredient-type-" + i} className="form-control d-inline-block mr-2 mb-2" defaultValue={ingredient.type} required>
                    ...
                    <button className="btn btn-danger mb-3" type="button"
                      onClick={() => {
                        setIngredients((prev) => {
                          const ret = [...prev];
                          ret.splice(
                            prev.findIndex(([id]) => id === ingredientId), 1
                          );
                          return ret;
                        });
                      }}
          ...
          {steps.map(([stepId, step], i) => (
            <div key={stepId} className="col-sm-10 offset-2">
              <div className={"row mb-3 " + (i === steps.length - 1 ? "" : "border-bottom")}>
                <div className="col-md-6">
                  <input type="text" name={"step-content-" + i} className="form-control d-inline-block mr-2 mb-2" placeholder={i + 1 + ". lépés"} defaultValue={step.content} required />
                </div>
                <div className="col-md-2">
                  <input type="number" name={"step-timer-" + i} className="form-control d-inline-block mr-2 mb-2" placeholder="Idő" defaultValue={step.timer} required />
                </div>
                <div className="col-md-2">
                    <div className="w-100">
                      <button className="btn btn-danger mb-3" type="button"
                        onClick={() => {
                          setSteps((prev) => {
                            const ret = [...prev];
                            ret.splice(
                              prev.findIndex(([item, _]) => item === stepId), 1
                            );
                            return ret;
                          });
                        }}
        ...
        <div className="form-group row pb-3 border-bottom">
          ...
          <div className="col-md-5">
            <img src={`${process.env.REACT_APP_BACKEND_URL}/static/images/${recipe.imageURL}`} className="img-thumbnail" />
          </div>
        </div>
